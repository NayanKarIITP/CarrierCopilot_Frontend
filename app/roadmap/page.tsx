...
import api from "@/lib/api"; // ✅ Use central API helper
...
export default function RoadmapPage() {
  ...
  const [roadmapSteps, setRoadmapSteps] = useState<RoadmapStep[]>([]);
  const [selectedRole, setSelectedRole] = useState("Full Stack Developer");
  const [userSkills, setUserSkills] = useState<string[]>([]); // ✅ Store user skills
  ...
  const handleGenerate = async () => {
    ...
    try {
      // ✅ FIX: Send BOTH role and skills so Python knows the context
        ...
        <div className="space-y-4">
          <div className="px-2 md:px-0 border-b border-gray-200 dark:border-border pb-6">
            <h1 className="text-3xl md:text-4xl font-bold mb-2 text-gray-900 dark:text-white flex items-center gap-3">
              Your AI Growth Roadmap <Sparkles className="text-yellow-500" size={28} />
            </h1>
          ...
          <Card className="p-6 flex flex-col md:flex-row gap-4 items-center bg-blue-50/50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-900">
            ...
            <button 
                onClick={handleGenerate} 
                disabled={generating}
                className="mt-6 md:mt-0 w-full md:w-auto bg-blue-600 text-white px-8 py-2.5 rounded-lg font-bold hover:bg-blue-700 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-blue-200 dark:shadow-none transition-all hover:scale-105 active:scale-95"
            >
                {generating ? <Loader2 className="animate-spin" size={20}/> : <Rocket size={20}/>}
