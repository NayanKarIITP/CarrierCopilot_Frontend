...
export default function PDFViewer({ fileUrl }: PDFViewerProps) {
  if (!fileUrl) {
    return (
      <div className="bg-white/50 dark:bg-muted/10 backdrop-blur-sm rounded-2xl border border-dashed border-gray-300 dark:border-border h-64 lg:h-full lg:min-h-[600px] flex flex-col items-center justify-center p-8 lg:sticky lg:top-8 text-center transition-colors">
        ...
        <p className="text-gray-900 dark:text-gray-200 font-medium">No Resume Uploaded</p>
        <span className="text-sm text-gray-500 dark:text-gray-400 mt-1">Upload a PDF to see the preview here</span>
      </div>
    )
  }
  ...
  return (
    <div className="bg-gray-900 dark:bg-black rounded-2xl border border-gray-800 dark:border-gray-800 overflow-hidden shadow-xl h-[500px] lg:h-[calc(100vh-120px)] lg:sticky lg:top-8 flex flex-col">
      
      {/* Header */}
      <div className="p-3 bg-gray-800 dark:bg-gray-900 border-b border-gray-700 flex justify-between items-center">
        <h3 className="font-semibold text-gray-200 text-sm flex items-center gap-2">
            <FileText size={16} /> Resume Preview
        </h3>
        <a 
